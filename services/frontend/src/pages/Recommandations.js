...
import { FiTrendingUp, FiStar, FiUser, FiSearch, FiZap, FiAward, FiBookOpen, FiPlay } from 'react-icons/fi';
...
function Recommandations() {
  ...
  const handleTrain = async () => {
    try {
      ...
      setTrainMsg(`✅ Modèle entraîné ! RMSE: ${result.metriques.rmse}, MAE: ${result.metriques.mae}`);
    } catch (err) { setTrainMsg(`❌ ${err.message}`); }
  };
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={handleTrain}><FiPlay /> Entraîner le modèle</button>
      </div>
      <p className="subtitle">Recommandations personnalisées par Machine Learning</p>

      {trainMsg && <div className={trainMsg.startsWith('✅') ? 'success' : 'error'}>{trainMsg}</div>}

      <div className="reco-input">
        <div className="input-group">
          <FiUser size={18} />
          <input type="number" value={userId} onChange={(e) => setUserId(e.target.value)} min="1" placeholder="ID" />
        </div>
            ...
            {recommandations.recommandations.map((rec, i) => (
              <div className="card reco-card" key={i}>
                ...
                <div className="card-content">
                  <h3>{rec.titre}</h3>
                  <p className="meta">✍️ {rec.auteur} • 📂 {rec.categorie}</p>
