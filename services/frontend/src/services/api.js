...
export const api = {
  login: async (email, password) => {
    const res = await fetch(`${API.auth}/login`, { method: 'POST', headers: headers(false), body: JSON.stringify({ email, password }) });
    if (!res.ok) throw new Error((await res.json()).error);
    return res.json();
  },
  register: async (data) => {
    const res = await fetch(`${API.auth}/register`, { method: 'POST', headers: headers(false), body: JSON.stringify(data) });
    if (!res.ok) throw new Error((await res.json()).error);
    return res.json();
  },
  getMe: async () => {
  ...
  emprunter: (userId, livreId) => fetch(API.emprunts, {
    method: 'POST', headers: headers(false), body: JSON.stringify({ email: getEmail(), livre_id: livreId })
  }).then(r => r.json()),
